...
imgPair.moving.input = imread('composite_post.tif'); % set to post
imgPair.static.input = imread('composite_pre.tif'); % set to pre; this is ground truth
...
timestamp = [num2str(currTime(1)*10000+ currTime(2) * 100 + currTime(3)) '-' ...
    num2str(currTime(4),'%.2d') ':' num2str(currTime(5),'%.2d')]
